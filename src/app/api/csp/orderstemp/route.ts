...
export const POST = async (req: Request) => {
  const { csp_order_id, company_id, customer_id, csp_file_url, csp_file_name } = await req.json();
    ...
    const queryStr = `
					...
					BEGIN
						CSP000.ins_csp_orders(
							:i_csp_uuid, :i_company_id, :i_customer_id, :i_csp_file_url, :i_csp_file_name, :o_msg);
					END; `;
